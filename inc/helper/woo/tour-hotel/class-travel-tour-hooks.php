...
class Hotel_Tour_WC_Hooks {
	...
	public function product_class( $classname, $product_type, $post_type, $product_id ): string {
		if ( 'travel-tour' == get_post_type( $product_id ) ) {
			$classname = 'WC_Product_Tours';
		}
	...
	public function order_item_line( $item, $cart_item_key, $values, $order ) {
		if ( 'travel-tour' === get_post_type( $values['product_id'] ) ) {
			$item->add_meta_data( '_trave_tour_id', $values['product_id'], true );
		}
	}
	...
	public function get_classname_up_game_wc_order( $classname, $item_type, $id ) {
		if ( in_array( $item_type, array( 'line_item', 'product' ) ) ) {
			$travel_tour_id = wc_get_order_item_meta( $id, '_trave_tour_id' );
			if ( $travel_tour_id && 'travel-tour' == get_post_type( $travel_tour_id ) ) {
				$classname = 'WC_Order_Item_Travel_Tour';
			}
		}
	...
	public static function instance(): Hotel_Tour_WC_Hooks {
